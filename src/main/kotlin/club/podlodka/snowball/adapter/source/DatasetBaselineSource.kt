...
/** A fixture row rejected before it could become a scenario, with the reason. */
...
class DatasetBaselineSource(
    ...
    override fun load(): List<BaselineRecord> {
        open().use { reader ->
            ...
            val records =
                lines.drop(1).mapIndexed { index, line ->
                    parse(header, split(line), index + 2)
                }
    ...
    private fun parse(
        ...
        if (values.size != header.size) {
            throw FixtureRejection("line $lineNumber has ${values.size} fields, expected ${header.size}")
        }
        ...
        try {
            ...
            throw FixtureRejection("line $lineNumber rejected: ${failure.message}")
        }
    }

    /**
     * A split that is not by time lets the benchmark measure memorised homework, so a fixture that
     * interleaves the two sets is refused outright rather than quietly measured.
     */

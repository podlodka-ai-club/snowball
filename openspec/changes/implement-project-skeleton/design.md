...
7. **Money stays as the schema types it.** The schemas declare `price`, `cost`, and `gross_profit` as JSON numbers, so the models carry them as `Double` and leave `BigDecimal` rounding policy to the Market Simulator, which is where `docs/market-simulator/README.md` specifies it. The skeleton must not pre-empt that rounding decision. This is a knowing trade: `Double` cannot represent every JSON number the schema allows, and `Int` for `stock`, `baseline_sales`, and `units_sold` is narrower than an unbounded JSON `integer`. Both are far outside anything an FMCG scenario produces, and widening either later is a type change in one file.
8. **The models enforce the schema's own invariants, not business rules.** `minLength: 1`, `exclusiveMinimum: 0` on `price`, and `minimum: 0` on `cost`, `stock`, `baseline_sales`, and `units_sold` are part of the contract, not of any component's logic, so they are checked in `init` blocks and hold for direct construction as well as for parsing. Jackson is also configured to reject what the schemas reject and its own defaults would accept: scalar coercion, float-to-integer coercion, and an explicit `null` in a field the schema only permits to be absent. Anything beyond the schema - a plausible price range, a category whitelist - belongs to the component that owns the meaning.

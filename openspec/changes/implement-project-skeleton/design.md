...
2. **Hand-written models, not generated ones, pinned by the schemas themselves.** Three small schemas do not repay a generator plugin in the build, and generated names would not match the ports the components already reference by name in their specs. What keeps a hand-written model honest is the test layer: the committed schemas are compiled into real validators, every committed example is checked against its schema, everything a model serializes is checked against its schema, and every negative case asserts that the schema *and* the model reject the same document.

   That last part is the point. A negative test that only says "the model rejects `snow`" keeps passing after somebody adds `snow` to the committed enum, leaving the model and the contract silently disagreeing - verified by making exactly that edit and watching the suite stay green before this was added, and go red after.
...
9. **The boundary holds on the way out as well as on the way in.** A model built in Kotlin rather than parsed can just as easily produce a document the schema rejects, and nothing upstream would catch it. Two cases are real: java-time accepts years beyond four digits, which print as `+10000-07-18`, and offsets carrying seconds, which print as `+01:02:03`; neither is a valid RFC 3339 `date` or `date-time`. Both are checked on construction, and the tests validate what each model serializes against the committed schema.

   The same reasoning covers cross-field agreement. The schemas cannot express that `decision_id`, `scenario_id`, and the scenario snapshot describe one chain - to a schema they are two non-empty strings and an object - so an event assembled field by field can carry parts of three unrelated events. Each downstream event therefore gets a constructor that takes the upstream event and copies the identity and the snapshot out of it, making the correct construction the convenient one. The field-by-field constructor stays for deserialization and for tests.

10. **`BigDecimal` equality is scale-sensitive, and that is left alone.** `320.0` and `320.00` are unequal as models, and unequal in sets and maps, though `compareTo` says they are the same amount. For a type whose job is to mirror a document exactly this is the correct behavior - the two spellings really are different documents - and normalising the scale would break the lossless round trip. It matters in practice because the committed example carries scale 1 while the simulator computes at scale 2, so a test pins the behavior and the KDoc says plainly: compare amounts with `compareTo`, compare events by their ids.
...
11. **CI runs exactly the acceptance command.** One workflow, one job, `spotlessCheck` before `build`, so a formatting failure is reported as such instead of as a compile error.
